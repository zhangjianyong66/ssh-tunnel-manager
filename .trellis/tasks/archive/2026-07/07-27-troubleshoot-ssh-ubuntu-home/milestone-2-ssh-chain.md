...
- [x] M2-T1：为每个 SSH 会话生成并持有 `0600` 临时 OpenSSH 配置；所有 check、exit、execute 和 forward 调用统一携带 `-F`。
- [x] M2-T2：实现一层依赖解析、跳板 ControlMaster 复用、依赖目标断开门禁、退出拓扑顺序和自动重连接线。
- [x] M2-T3：扩展连接请求与 SSH 错误结构，按 `stageHost` 分阶段处理并分别保存跳板/目标密码和私钥口令；覆盖用户名变更与删除凭据清理。
- [x] M2-T4：扩展 askpass 与错误分类，捕获未知主机指纹、严格匹配用户确认，并永久拒绝已变化密钥；补齐秘密脱敏与并发测试。
