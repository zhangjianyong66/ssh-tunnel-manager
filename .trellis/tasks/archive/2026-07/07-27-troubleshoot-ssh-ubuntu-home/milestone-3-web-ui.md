...
- [x] M3-T1：调整服务器表格，展示来源、目标和跳板；增加项目 Host 的新增、编辑、删除入口及稳定加载状态。
- [x] M3-T2：实现结构化 Host 对话框、字段校验、单层跳板筛选、不可变 Alias 和删除凭据确认。
- [x] M3-T3：把连接流程改为基于 `stageHost` 的凭据挑战与指纹确认状态机，覆盖取消、重试、密钥变化和引用失效提示。
- [x] M3-T4：补充模板/API 契约测试，并在桌面与移动视口检查文本、表格、对话框和操作控件无重叠。
